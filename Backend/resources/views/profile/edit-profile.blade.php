...
@section('content')
    <form action="" method="POST" enctype="multipart/form-data" id="form-edit-profile">
        <div class="card mb-4">
                            ...
                            <div class="col-xlg-8">
                                <select name="gender" class="form-control" id="gender">
                                    <option value="{{\App\Models\User::MALE}}"
                                        {{ $user->gender == \App\Models\User::MALE ? 'selected' : '' }}>Nam
                                    </option>
                                    <option value="{{\App\Models\User::FEMALE}}"
                                        {{ $user->gender == \App\Models\User::FEMALE ? 'selected' : '' }}>Nữ
                                    </option>
                                    <option value="{{\App\Models\User::OTHER_GENDER}}"
                                        {{ $user->gender == \App\Models\User::OTHER_GENDER ? 'selected' : '' }}>Khác
                                    </option>
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="image-preview-wrapper">
                                    @if(!is_null($user->image))
                                        <div class="image-preview"
                                             style="background-image: url({{ route('content.show', base64_encode('app/' . $user->image)) }});
