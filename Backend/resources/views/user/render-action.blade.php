...
<td class="">
    @if(Auth::user()->role == \App\Models\User::ROLE_ADMIN)
        <div class="d-flex jt-cont-sp-bw" >
            ...
            @if($user->active == 1)
                <a data-toggle="tooltip" data-coreui-placement="bottom" data-id="{{$user->id}}" data-coreui-original-title="Đã kích hoạt" class="user-unlock">
                    <i class="icon-lock fas fa-user-unlock fa-lg mr-3 cl-green"></i>
                </a>
            @elseif($user->active == 0)
                <a data-toggle="tooltip" data-coreui-placement="bottom" data-id="{{$user->id}}" data-coreui-original-title="Vô hiệu hóa" class="user-lock">
                    <i class="icon-user-lock fas fa-user-lock fa-lg mr-3 cl-red"></i>
                </a>
            @endif
            @if(\App\Models\User::ROLE_ADMIN != $user->role)
            <a data-toggle="tooltip" data-coreui-placement="bottom" data-id="{{$user->id}}" data-coreui-original-title="Xóa" class="user-delete" >
